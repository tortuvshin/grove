/**
 * @grove-dev/ui — formatting helpers (V1, dependency-free).
 *
 * Mirrors `open-apps/src/lib/format.ts` so the openapps-derived
 * component library and any other consumer (Astro, Svelte, Next.js)
 * can share the same string shaping.
 *
 * Keep these functions pure so they can run in both SSR and
 * client-side scripts.
 */
...
/**
 * Format a number with thousands separators. 1234 → "1,234". Returns
 * "—" for non-finite / nullish input. Mirrors openapps `formatNumber`.
 */
