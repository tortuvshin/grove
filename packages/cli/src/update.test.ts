...
describe('grove update', () => {
  it('reports a fresh scaffold as fully unchanged', async () => {
    ...
    expect(summary.plan.unchanged).toHaveLength(81);
  ...
  it('adopts a lockfile-less project without overwriting local edits', async () => {
    ...
    expect(lock.files.length).toBe(81);
  });
  ...
  it('adoption is idempotent — the second run reports no adoption and no work', async () => {
    ...
    expect(second.plan.unchanged).toHaveLength(81);
  ...
  it('applies upstream changes, flags conflicts, and refreshes the lock', async () => {
    ...
    expect(summary.plan.unchanged).toHaveLength(79);
    expect(summary.applied).toEqual([UNTOUCHED]);
    expect(summary.preserved).toEqual([EDITED]);
    ...
    expect(again.plan.unchanged).toHaveLength(80);
  });
  ...
  it('--force takes the upstream side of a conflict and then settles', async () => {
    ...
    const again = await runUpdate({ cwd, from, check: true });
    expect(again.plan.unchanged).toHaveLength(81);
  });
