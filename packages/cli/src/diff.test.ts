...
describe("classify", () => {
  it("returns unchanged when all three hashes match", () => {
    expect(
      classify({ installed: h("a"), lock: h("a"), registry: h("a") }),
    ).toBe("unchanged");
  });

  it("returns upstream_changed when only registry differs", () => {
    expect(
      classify({ installed: h("a"), lock: h("a"), registry: h("b") }),
    ).toBe("upstream_changed");
  });

  it("returns locally_modified when only installed differs from lock", () => {
    expect(
      classify({ installed: h("c"), lock: h("a"), registry: h("a") }),
    ).toBe("locally_modified");
  });

  it("returns conflict when both sides moved", () => {
    expect(
      classify({ installed: h("c"), lock: h("a"), registry: h("b") }),
    ).toBe("conflict");
  });

  it("returns new when only registry has the file", () => {
    expect(
      classify({ installed: null, lock: null, registry: h("a") }),
    ).toBe("new");
  });

  it("returns removed when only consumer has the file", () => {
    expect(
      classify({ installed: h("a"), lock: h("a"), registry: null }),
    ).toBe("removed");
  });
});
