...
export function buildReadmeCommand(): Command {
  ...
  cmd
    ...
    .option(
      "--stdout",
      "Print to stdout instead of writing to README.md (for CI dry-runs).",
    )
    .option("--path <path>", "README path relative to cwd", DEFAULT_README_PATH)
    .option(
      "--check",
      "Exit with code 1 when the rendered block differs from the existing README.",
    )
    .action(
      async (opts: {
        stdout?: boolean;
...
async function loadRecords(
  ...
  for (const file of files) {
    ...
    const repoMeta =
      (github.repository as Record<string, unknown> | undefined) ?? {};
    ...
    const stars =
      typeof github.stars === "number"
        ...
        : typeof repoMeta.stargazers_count === "number"
          ...
          : undefined;
    const license =
      typeof github.license === "string"
        ? (github.license as string)
        : undefined;
    out.push({
...
async function loadCategories(
  cwd: string,
  taxonomyDir: string,
): Promise<AwesomeReadmeCategory[]> {
  const path = resolve(cwd, taxonomyDir, "categories.yml");
  const raw = (parseYaml(await readFile(path, "utf8")) ?? []) as Array<
