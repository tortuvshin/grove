...
describe('parseAuditBlock', () => {
  ...
  it('parses pages only', () => {
    const result = parseAuditBlock(
      parseObjectExpression('{ pages: [{ path: "/", type: "home", label: "Home" }] }'),
    );
...
describe('parsePageEntry', () => {
  it('parses all 3 fields', () => {
    const entry = parsePageEntry(
      parseObjectExpression('{ path: "/foo", type: "directory", label: "Foo" }'),
    );
