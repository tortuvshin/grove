...
describe("pickCleanupCandidates — pure predicate", () => {
  ...
  it("returns records whose health.status is 'unknown' or 'needs_review'", () => {
    const a = makeProjectResource({ slug: "a", health: { status: "unknown", maturity: "unknown", tier: "experimental", visibility: "keep", cleanupCandidate: false, staleReason: null, confidence: "low", reasons: [] } });
    const b = makeProjectResource({ slug: "b", health: { status: "needs_review", maturity: "useful", tier: "listed", visibility: "keep", cleanupCandidate: false, staleReason: null, confidence: "medium", reasons: [] } });
    const c = makeProjectResource({ slug: "c", health: { status: "active", maturity: "mature", tier: "curated", visibility: "keep", cleanupCandidate: false, staleReason: null, confidence: "high", reasons: [] } });
  ...
  it("returns an empty array when no records need cleanup", () => {
    const a = makeProjectResource({ health: { status: "active", maturity: "mature", tier: "curated", visibility: "keep", cleanupCandidate: false, staleReason: null, confidence: "high", reasons: [] } });
    const b = makeProjectResource({ slug: "b", health: { status: "mature", maturity: "mature", tier: "curated", visibility: "keep", cleanupCandidate: false, staleReason: null, confidence: "high", reasons: [] } });
...
describe("cleanupStale — filesystem integration (real tmpdir)", () => {
  ...
  it("reads records, applies pickCleanupCandidates, writes cleanup-report.json", async () => {
    ...
    const written = JSON.parse(await readFile(path, "utf8")) as { totalCandidates: number; candidates: Array<{ slug: string }> };
  ...
  it("returns an empty report when the records directory does not exist", async () => {
    ...
    const noRecordsConfig = {
      site: { name: "test", tagline: "test" },
      paths: { recordsDir: "data/does-not-exist", generatedDir: "data/generated", health: "data/health.yml", decisions: "data/decisions.yml" },
