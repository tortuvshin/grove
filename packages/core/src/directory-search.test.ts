...
describe('directory discovery state', () => {
  it('filters label-backed and curator-assigned lenses through the same canonical engine', () => {
    expect(
      filterRecords(records, filtersFromSearchParams(new URLSearchParams('label=hot'))).map(
        (item) => item.slug,
      ),
    ).toEqual(['trending']);
    expect(
      filterRecords(
        records,
        filtersFromSearchParams(new URLSearchParams('lens=good-to-learn')),
      ).map((item) => item.slug),
    ).toEqual(['learn']);
  });
  ...
  it('combines facets with a lens using AND across dimensions', () => {
    const filters = filtersFromSearchParams(
      new URLSearchParams('label=hot&category=agents&stack=python'),
    );
  ...
  it('keeps tags separate from category and stack facets', () => {
    ...
    expect(
      filterRecords(tagged, filtersFromSearchParams(new URLSearchParams('tag=self-hosted'))).map(
        (item) => item.slug,
      ),
    ).toEqual(['agent-ui']);
  });
  ...
  it('builds single-select lens links that keep refinements but reset the view', () => {
    ...
    const current = new URLSearchParams(
      'q=agent&category=agents&label=new&status=quiet&page=3&sort=alphabetical',
    );
    expect(hrefForLens('hot', current, '/projects')).toBe(
      '/projects?q=agent&category=agents&label=hot',
    );
    expect(hrefForLens('all', current, '/projects')).toBe('/projects?q=agent&category=agents');
    expect(
      isLensActive('hot', new URL('https://example.com/projects?label=hot').searchParams),
    ).toBe(true);
    expect(
      isLensActive('all', new URL('https://example.com/projects?category=agents').searchParams),
    ).toBe(true);
  });
...
function licensedRecord(
  ...
  base.github =
    options.githubLicense === undefined
      ? undefined
...
describe('license filter', () => {
  it('matches records with a curated lowercase SPDX id', () => {
    ...
    expect(
      filterRecords(records, filtersFromSearchParams(new URLSearchParams('license=mit'))).map(
        (r) => r.slug,
      ),
    ).toEqual(['curated']);
  });
  ...
  it('falls back from curated to GitHub when curated is absent', () => {
    ...
    expect(
      filterRecords(
        records,
        filtersFromSearchParams(new URLSearchParams('license=apache-2.0')),
      ).map((r) => r.slug),
    ).toEqual(['github-only']);
  });
