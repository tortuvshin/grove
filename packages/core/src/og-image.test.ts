...
describe('buildOgImages', () => {
  it('writes the full og/ tree and skips unchanged images on rebuild', {
    timeout: 60_000,
  }, async () => {
    ...
    const input = {
      ...
      taxonomies: [{ facet: 'category' as const, id: 'photos', label: 'Photos', count: 4 }],
  ...
  it('regenerates when the manifest is corrupt instead of failing', {
    timeout: 60_000,
  }, async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'grove-og-'));
    roots.push(cwd);
