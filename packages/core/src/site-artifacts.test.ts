...
import { buildOgImageSvg, buildSiteArtifacts } from './site-artifacts.js';
...
describe('site artifacts', () => {
  ...
  it('updates Grove-owned files but preserves consumer-owned custom files', async () => {
    ...
    expect(await readFile(join(cwd, 'public', 'robots.txt'), 'utf8')).toBe('# custom robots\n');

    const second = await buildSiteArtifacts(cwd, config, { totalRecords: 3 });
    expect(second.ogImageWritten).toBe(true);
    expect(await readFile(join(cwd, 'public', 'og-image.svg'), 'utf8')).toContain('3 apps');
  });
});
