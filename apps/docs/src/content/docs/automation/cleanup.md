...
`grove cleanup` reads every record's `health` block (from the GitHub sync cache, else inline, else `data/health.yml`), filters for the ones that need a human look, and writes `data/generated/cleanup-report.json`. **The command never deletes or edits a record.** It surfaces a triage list; a curator acts on it via the record's YAML or `data/decisions.yml`.
...
`cleanupStale()` takes its records from the same normalizer the build uses (`loadNormalizedRecords`, `packages/core/src/normalize-records.ts`): `github` and `health` from the record's GitHub sync cache entry when there is one (else the block inline on the record, else its `data/health.yml` entry), with `data/overrides.yml` and `data/decisions.yml` applied. A decision only changes `health.visibility`, so a record with a `keep` decision can still show up in the cleanup report if its `health.cleanupCandidate` is `true` or its status needs review. A record with no observed health at all is never a candidate, even when a decision gave it a placeholder `unknown` health block to carry its visibility. Records that fail the schema are left out; `grove check` reports them.

`classifyHealth()` (`packages/core/src/health.ts`) is what derives that block from GitHub metadata. `grove sync github` runs it and writes the result into each record's GitHub sync cache entry (`data/cache/github/<slug>.json`) when `integrations.github.health` is enabled; otherwise health is hand-authored inline or in `data/health.yml`. The build and `grove cleanup` share the whole lookup, `data/health.yml` fallback included. See [Maintain health signals](/content/health-classification/).
