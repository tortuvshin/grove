...
<Aside type="caution" title="Editing the `health` block by hand">
  Auto-derived from GitHub metadata on every sync run. Hand edits get overwritten. If you disagree with the auto-derived health, write a [decision](/concepts/decisions/).
</Aside>
...
The slug must match the filename. `kind` is `project` for every record today — `grove init` always scaffolds the `project-directory` blueprint, the only one currently supported. `category` must be a valid id from `data/taxonomy/categories.yml` (otherwise the record fails to validate).
...
- **Add a record** — create `data/records/<slug>.yml` with at least `kind`, `name`, `description`.
