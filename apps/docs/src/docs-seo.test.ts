...
const sidebarSlugs = (items: unknown[]): string[] =>
  items.flatMap((item) => {
    ...
    return [...(entry.slug ? [entry.slug] : []), ...(entry.items ? sidebarSlugs(entry.items) : [])];
  });
...
describe('llms.txt builders', () => {
  ...
  it('builds trailing-slash absolute URLs from collection ids', () => {
    expect(docSlugToUrl('introduction', site.url)).toBe('https://withgrove.dev/introduction/');
...
describe('generated raster assets', () => {
  const png = async (path: string) => sharp(resolve(docsRoot, 'public', path)).metadata();
...
describe('per-page head coverage', () => {
  it('registers the Head override with robots + article JSON-LD', async () => {
    const head = await readFile(resolve(docsRoot, 'src/starlight/Head.astro'), 'utf8');
