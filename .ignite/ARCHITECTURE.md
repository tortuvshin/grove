...
- `site` is both the real AI demo and the only scaffold copied by `grove init`.
...
`grove init` copies the canonical `site/` once. After initialization, page and style changes are user code. `grove sync` updates generated metadata and public build artifacts only; it is not a template updater.
