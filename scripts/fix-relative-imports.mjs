...
const ROOTS = [resolve('apps/example/src'), resolve('packages/registry/default')];
...
function normalizeSpec(spec) {
  ...
  return spec.replace(/\/([A-Z][A-Za-z]*)(?=\.[a-z]+$|\/|$)/g, (whole, name) => `/${kebab(name)}`);
}
...
async function main() {
  ...
  for (const root of ROOTS) {
    for await (const file of walk(root)) {
      ...
      const replaced = source.replace(/from\s+["']([^"']+)["']/g, (whole, spec) => {
