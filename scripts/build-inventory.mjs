...
const KIND_BY_DIR = {
  "ui": (file) =>
    KNOWN_LIGHT_LOGIC.has(file) || hasClientBehavior(file)
      ? "composition"
      : "primitive",
  "layouts": () => "layout",
  "components": (file) => {
...
async function build() {
  ...
  for await (const full of walk(sourceDir)) {
    ...
    files.push({
      ...
      hasBusinessLogic:
        KNOWN_CLIENT_CONTROLLERS.has(normalized) ||
        KNOWN_LIGHT_LOGIC.has(normalized),
  ...
  return `${JSON.stringify(
    {
      ...
      forbiddenImportCount: files
        .map((f) => f.forbiddenImports.length)
        .reduce((a, b) => a + b, 0),
      files,
    },
    null,
