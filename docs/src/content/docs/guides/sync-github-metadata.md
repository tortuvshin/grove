...
The cleanup step (`grove cleanup stale`, see [Maintain health signals](/guides/maintain-health-signals/)) flags all of these. If the report has a sudden spike in `archived` or `unavailable` candidates, check the upstream before writing decisions — sometimes it's a transfer, and the new owner is actively maintaining the project.
