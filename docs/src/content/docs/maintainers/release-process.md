...
- **The docs site (`docs/`)** is not in the lockstep release. The docs site is built and deployed separately (to grove.dev) and does not have a version number tied to the `@grove-dev/*` packages. A docs change does not require a release.
